...
const getTotalAmount = cart => cart.reduce((total, item) => total + (item.quantity * item.unit_cost), 0.0);


const getTotalItemCount = cart => cart.reduce((total, item) => total + item.quantity, 0);


const handlePercentage = (cart, coupon, response) => {
  const totalAmount = getTotalAmount(cart);
  let discount = (totalAmount * coupon.value) / 100;
...
const applyCoupon = (cartObj, couponCode, outletId) => {
  ...
  for (let i = 0; i < availableCoupons.length; i += 1) {
  ...
  const result = checkValidCoupon(coupon, outletId);
